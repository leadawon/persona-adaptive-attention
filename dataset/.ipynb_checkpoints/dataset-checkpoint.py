...
class PersonaChatDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, persona_query_token, turns=-1, max_context_turns=-1,
                 ...
                 extend_candidates=False, num_candidates=0, dawon_flag="error"):
        self.path = data_path
        ...
        #dawon custom ->
#         print(type(self.turns_data))
#         print(os.path.abspath(__file__)) #/home/leadawon5/decs_jupyter_lab/gitfiles/persona-adaptive-attention/dataset/dataset.py
        self.turns_data = []
        with open(f"./bigdata/msc/contains_persona_jsonl/session_3_{dawon_flag}.jsonl","r") as infile:
            for dialog_idx,json_line in enumerate(infile):
                json_file = json.loads(json_line)
                for session_idx in range(3):
                    prev_strs = []
                    for utterance_idx,utterance in enumerate(json_file[f'session{session_idx+1}']): ## 이거 바꿔야 할수도 있다. 앞에 Q,R 붙여야 할수도 있다.
                        dic = {'input':[],'input_str':'','target':'','persona':'','persona_list':[],'candidates':["sample utterance1","sample utterance2","sample utterance3"]}
                        if(utterance_idx > 3 and utterance_idx < 14):
                            dic['input'] = copy.deepcopy(prev_strs)
                            sep_str = ' '+self.persona_query_token+' '
                            dic['input_str'] = sep_str.join(prev_strs)
                            dic['target'] = utterance
                            if (utterance_idx % 2 == 0):
                                dic['persona'] = ' '.join(json_file[f'session{session_idx+1}_persona'][0])
                                dic['persona_list'] = copy.deepcopy(json_file[f'session{session_idx+1}_persona'][0])
                            else:
                                dic['persona'] = ' '.join(json_file[f'session{session_idx+1}_persona'][1])
                                dic['persona_list'] = copy.deepcopy(json_file[f'session{session_idx+1}_persona'][1])
                                dic['candidates'].append(utterance)
                            for k,v in dic.items():
                                ...
                                self.turns_data.append(copy.deepcopy(dic))
                        prev_strs.append(utterance)
                    
                
                
        #print(self.persona_query_token)
        #assert False
        #dawon custom <-
...
#             #dawon custom
#             print("Persona list element:", persona_list_element)
